...
export async function TournamentSocialPublic({ tournament }: { tournament: Tournament }) {
  const visible = SOCIAL_DEFS.map((s) => ({
    ...s,
    href: publicSocialHref(tournament, s.key),
    subtext: publicSocialSubtext(tournament, s.key),
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {visible.length === 0 ? (
        <p className="text-sm text-zinc-600">No social links are available for this tournament yet.</p>
      ) : (
              ...
              <li key={s.key}>
                ...
                >
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="block text-xs text-zinc-500">{s.subtext}</span>
                  </span>
                </Link>
              </li>
            );
